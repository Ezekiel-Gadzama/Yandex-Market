...
export default function MarketingEmails() {
  ...
  const createFromFileMutation = useMutation({
    mutationFn: (file: File) => marketingEmailsApi.createFromFile(file),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['marketing-templates'] })
      setShowModal(false)
      setEditingTemplate(null)
                    ...
                    <div className="flex items-center border border-gray-300 rounded-md overflow-hidden">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          if (fmt) {
                            marketingEmailsApi.export(template.id, fmt).catch(() => alert('Download failed'))
                            e.target.value = ''
                          }
                        }}
                      >
            ...
            <form id="marketing-form" onSubmit={handleSubmit} className="space-y-4 flex-1 overflow-y-auto">
              {!editingTemplate?.id && (
                <div className="pb-4 border-b border-gray-200">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      id="marketing-upload-file"
                      type="file"
                      accept=".txt,.pdf"
                      className="hidden"
                      onChange={(e) => {
                        const file = e.target.files?.[0]
                        if (file) createFromFileMutation.mutate(file)
                        e.target.value = ''
                      }}
                    />
                    <button
                      type="button"
                      onClick={() => document.getElementById('marketing-upload-file')?.click()}
                      disabled={createFromFileMutation.isPending}
                      className="inline-flex items-center px-3 py-2 border border-gray-300 rounded-md text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50"
                    >
                      <Upload className="h-4 w-4 mr-2" />
                      {createFromFileMutation.isPending ? 'Uploading...' : 'Upload TXT or PDF'}
                    </button>
